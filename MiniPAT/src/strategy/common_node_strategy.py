"""
通用 AST 节点分析策略模块：提供对各类 JavaScript AST 节点（声明、表达式、语句等）的遍历与解析函数。
"""
                    ...
                    if variable_identifier is not None:
                        if 'getApp' in variable_identifier:
                            ...
                            if "app.js" in tmp_context.name:
                                if len(variable_identifier.split("getApp.")) > 1:
                                    variable_identifier = variable_identifier.split("getApp.")[1]
                                    # 降级为 debug，避免每次 getApp 解析都产生 info 日志
                                    logger.debug(variable_identifier)
                                else:
                                    variable_identifier = 'getapp'
                            else:
...
def call_expression_analysis(call_expression: dict, context, mini_program: MiniProgram):
    ...
    for argument in arguments:
        if argument['type'] == 'Literal':
            ...
            binary_expression_analysis(argument, context)
    ...
    if 'object' in member_expression and 'property' in member_expression:
        ...
        if obj['type'] == 'Identifier':
            ...
            if value is not None:
                if 'name' in prop:
                    return str(value) + '.' + str(prop['name'])
                if 'value' in prop:
                    return str(value) + '.' + str(prop['value'])
                return value
            elif 'name' in prop:
                return prop['name']
            elif 'value' in prop:
